...
namespace MeshGeneration {
    ...
    public class TrackMeshGeneration {
        ...
        public TrackMeshGeneration(Path path, PolygonCollider2D trackShape, float width, float railWidth, float stepSize = 0.1f) {
            
            this.path = path;
            this.width = width;
            this.stepSize = stepSize;

            shapePoints = trackShape.points;
        ...
        public Mesh GenerateMesh() {
            ...
            CombineInstance[] combine = new CombineInstance[nSegments*2];
            ...
            for (var i = 0; i < nSegments; i++) {
                ...
                combine[i + nSegments].mesh = MakeLineMesh(sampleSpace, segment, -sideVector);
                combine[i + nSegments].transform = Matrix4x4.identity;
            }
        ...
        private Mesh MakeLineMesh(LinSpace sampleSpace, LineSegment lineSegment, Vector3 centerOffset) {
            ...
            Vector3 stepPlusOne = path.GetPoint(timeStepPlusOne) + rotatedOffset;
            
            Vector3 v0 = step + rotatedSegment.End;
            Vector3 v1 = step + rotatedSegment.Start;

            Vector3 v2 = stepPlusOne + rotatedSegment.End;
            Vector3 v3 = stepPlusOne + rotatedSegment.Start;
